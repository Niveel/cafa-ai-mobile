...
const PAGE_SIZE = 20;
const CARD_GAP = 10;
const IMAGE_HISTORY_MIN_REQUEST_GAP_MS = 1200;
const IMAGE_HISTORY_RATE_LIMIT_BACKOFF_MS = 12000;
...
export default function ImagesScreen() {
  ...
  useEffect(() => {
    void getAccessToken().then((token) => setAccessToken(token ?? null));
  }, []);
  ...
  const loadPage = useCallback(async (page: number, mode: 'replace' | 'append') => {
    ...
    const now = Date.now();
    if (now < historyRateLimitUntilRef.current) return;
    if (now - lastHistoryRequestAtRef.current < IMAGE_HISTORY_MIN_REQUEST_GAP_MS) return;
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : typed?.message ?? t('images.loadFailed');
      const code = (typed?.code ?? '').toUpperCase();
      const isRateLimited = typed?.status === 429 || code.includes('RATE_LIMIT') || message.toLowerCase().includes('too many');

      if (isRateLimited) {
        historyRateLimitUntilRef.current = Date.now() + IMAGE_HISTORY_RATE_LIMIT_BACKOFF_MS;
        showNotice(t('images.rateLimitedRetry'), 5000);
      } else {
        showNotice(message, 5000);
      }

      if (Date.now() - lastHistoryErrorLogAtRef.current > 4000) {
        console.log(`[images-history:error] endpoint=${API_BASE_URL}/images/history message="${message}"`);
        lastHistoryErrorLogAtRef.current = Date.now();
      }
      hapticError();
    } finally {
  ...
  const downloadImage = useCallback(async (item: ImageHistoryItem) => {
    const fallbackUrl = apiEndpoints.images.download(item.id);
    const resolvedUrl = resolveBackendAssetUrl(item.downloadUrl ?? item.imageUrl ?? fallbackUrl);
    if (!resolvedUrl) {
      showNotice(t('images.downloadOneFailed'));
      return;
    }
    ...
    try {
      ...
      const fileName = item.fileName || `cafa-ai-image-${item.id}.${extension}`;
      ...
      showNotice(t('images.downloadOneSuccess'));
      hapticSuccess();
    } catch (error) {
  ...
  const downloadAllAsZip = useCallback(async () => {
    ...
    setIsZipBusy(true);
    hapticSelection();
    showNotice(t('images.zipPreparing'));
    setZipProgress({
      visible: true,
      phase: 'preparing',
      downloadedBytes: 0,
    ...
    const startEndpoint = `${API_BASE_URL}${apiEndpoints.images.downloadZip.replace(/^\/api\/v1/i, '')}`;
    ...
    try {
      ...
      if (startPollUrl) {
        ...
        pollEndpoint = `${API_BASE_URL}${apiEndpoints.images.downloadZipJob(startJson.data.jobId).replace(/^\/api\/v1/i, '')}`;
      }
        ...
        if (response.status === 202 || contentType.includes('application/json')) {
          ...
          try {
            ...
            if (json.data?.status === 'failed') {
              throw new Error(message || t('images.zipFailed'));
            }
          ...
          if (attempt % 5 === 0) {
            showNotice(message || t('images.zipPreparing'));
          }
          await sleep(1700);
          continue;
        }
      ...
      if (!downloadEndpoint) throw new Error(t('images.zipFailed'));
      ...
      if (!result?.uri) {
        throw new Error(t('images.zipFailed'));
      }
      ...
      const message = error instanceof Error ? error.message : t('images.zipFailed');
      console.log(`[images-download-zip:error] endpoint=${downloadEndpoint || pollEndpoint || startEndpoint} message="${message}"`);
      showNotice(message, 5000);
      setZipProgress((prev) => ({ ...prev, visible: false, phase: 'idle' }));
      hapticError();
    } finally {
  ...
  const openDownloadedZipPath = useCallback(async () => {
    ...
    try {
      ...
      showNotice(t('images.zipOpenPathFailed'));
    }
        ...
        {Platform.OS === 'android' && zipProgress.visible ? (
          ...
          >
            <View className="flex-row items-center">
              ...
              <Text style={{ color: colors.textPrimary, marginLeft: 10, fontWeight: '700', fontSize: 13 }}>
                {zipProgress.phase === 'preparing'
                  ? t('images.zipPreparing')
                  : zipProgress.phase === 'complete'
                    ? t('images.zipCompleted')
                    : t('images.zipDownloading')}
              </Text>
            </View>
            ...
            <Text style={{ color: colors.textSecondary, fontSize: 11, marginTop: 6 }}>
              {t('images.zipDoNotCloseWarning')}
            </Text>

            {zipProgress.phase === 'complete' && zipProgress.fileUri ? (
              <TouchableOpacity
                accessibilityRole="button"
                accessibilityLabel={t('images.zipOpenPath')}
                accessibilityHint={t('images.zipOpenPathHint')}
